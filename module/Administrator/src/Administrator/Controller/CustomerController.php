...
class CustomerController extends MyController
{
    ...
    public function addCustomerAction(){
        if ($this->request->isPost()) {
            $params = $this->params()->fromPost();
            $result = Business\Customer::create($params);

            if(empty($result['success'])){
                return $this->getResponse()->setContent(json_encode(['st'=> -1, 'ms' => $result['messages']]));
            }else{
                return $this->getResponse()->setContent(json_encode(['st'=> 1, 'ms' => 'Tạo khách hàng thành công!' ,'data' => ['customer_id' => $result['id']]]));
            }
        }
    }
}